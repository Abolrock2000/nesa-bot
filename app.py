...
اینجا همیشه بازه برای حرف‌هایی
که گفتنش مستقیم سخته. 🌻❤️""", get_main_keyboard())
            return
        
        # اگر متن خالی بود
        if not text:
            send_message(chat_id, "🥺 یه چیزی بنویس که دلت می‌خواد بهش بگی ❤️")
            return
...
فقط چیزی رو بنویس
که واقعاً دلت می‌خواد بهش بگی. ❤️
...
def birthday_timer():
    ...
    while True:
        try:
            now = get_current_iran_time()
            if (now.month == BIRTH_MONTH and now.day == BIRTH_DAY and 
                now.hour == BIRTH_HOUR and now.minute == BIRTH_MINUTE):
                if not sent_today:
...
.card{
    padding:30px 22px;
    border-radius:28px;
    background:linear-gradient(145deg, rgba(70,15,30,.92), rgba(15,5,10,.95));
    border:1px solid rgba(255,150,170,.4);
    box-shadow:0 0 60px rgba(255,50,100,.25), inset 0 0 30px rgba(255,50,100,.08);
...
button{
    ...
    border-radius:30px;
    color:white;
    background:linear-gradient(45deg, #ff416c, #ff758c);
    font-size:17px;
    font-weight:bold;
    cursor:pointer;
    box-shadow:0 0 25px rgba(255,65,108,.5);
}
