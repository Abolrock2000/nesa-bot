...
import requests, json, datetime, os, threading, time, sqlite3
...
def add_queue(kind, content, caption="", title="", performer=""):
    with db() as c:
        c.execute("""INSERT INTO queue
            (kind,content,caption,title,performer,created_at)
            VALUES(?,?,?,?,?,?)""",
            (kind, content, caption or "", title or "",
             performer or "", now_iran().isoformat()))
        c.commit()
...
def pop_kind(kind):
    with db() as c:
        r = c.execute(
            "SELECT * FROM queue WHERE kind=? ORDER BY id LIMIT 1",
            (kind,)
        ).fetchone()
...
def clear_kind(kind=None):
    with db() as c:
        if kind:
            c.execute("DELETE FROM queue WHERE kind=?", (kind,))
        else:
...
def clean_file_id(value):
    """تلگرام file_id را بدون پیشوند file_id: دریافت می‌کند."""
...
def send_media(method, chat_id, field, value, caption="",
               title="", performer="", protect_content=False):
...
def send_audio(chat_id, value, caption="", title="",
               performer="❤️", protect_content=False):
    return send_media("sendAudio", chat_id, "audio", value, caption,
                      title, performer, protect_content)

def send_voice(chat_id, value, caption="", protect_content=False):
    value = clean_file_id(value)
    return api(
        "sendVoice",
        chat_id=chat_id,
...
def send_document(chat_id, value, caption="", protect_content=False):
    value = clean_file_id(value)
    return api(
        "sendDocument",
        chat_id=chat_id,
...
def send_file_to_chat(target, f, protect=False):
    ...
    if t == "photo":
        return send_photo(target, fid, f.get("caption", ""), protect)
    if t == "video":
        return send_video(target, fid, f.get("caption", ""), protect)
    if t == "audio":
        return send_audio(
            target, fid, f.get("caption", ""),
            f.get("title", "🎵"),
            f.get("performer", "❤️"),
            protect
        )
    if t == "voice":
        return send_voice(target, fid, f.get("caption", ""), protect)
    if t == "document":
        return send_document(target, fid, f.get("caption", ""), protect)
    return False
...
def log_activity(cid, action, chat):
    ...
    if cid == OWNER_CHAT_ID:
        return
    d = PARTNER_ACTIVITY.setdefault(cid, {
        "count": 0,
        "first_seen": now_iran(),
        "first_name": "",
        "last_name": "",
    ...
    send_message(
        OWNER_CHAT_ID,
        f"👀 تعامل جدید\n"
        f"👤 {d.get('first_name') or 'نامشخص'}\n"
        f"🆔 {cid}\n📌 {action}\n🔢 {d['count']}"
    )

def get_access(cid):
    return user_access.setdefault(str(cid), {
        "photos": False,
        "waiting": None,
        "mode": None,
        "target": None
...
def message_file(msg):
    ...
    if msg.get("audio"):
        ...
        return {
            ...
            "file_id": a["file_id"],
            "caption": caption,
            "title": a.get("title") or a.get("file_name") or "🎵",
            "performer": a.get("performer") or "❤️"
        }
    ...
    if msg.get("document"):
        ...
        if (
            ...
            or lower.endswith((".mp3", ".m4a", ".wav", ".ogg", ".flac"))
        ):
            return {
                "type": "audio",
                "file_id": x["file_id"],
        ...
        if (
            ...
            or lower.endswith((".mp4", ".mov", ".mkv", ".webm"))
        ):
            return {
                "type": "video",
                "file_id": x["file_id"],
...
def handle(cid, text, f=None):
    ...
    if cid == OWNER_CHAT_ID and text == "📢 مدیریت کانال":
        ...
        return send_message(
            cid,
            f"📢 مدیریت {CHANNEL_NAME}\n"
            f"⏰ ساعت فعلی: {setting('post_time', '10:10')}",
            channel_keyboard()
        )
    ...
    if text == "💬 چت دوطرفه":
        ...
        return send_message(
            cid,
            "💬 پیام، عکس، فیلم، موزیک، فایل یا ویس را ارسال کن.",
            chat_keyboard()
        )
    ...
    if u.get("mode") == "chat":
        if text == "📤 ارسال پیام":
            u["waiting"] = "chat_text"
            return send_message(cid, "💬 پیام را بنویس:", chat_keyboard())

        if text == "📤 ارسال عکس":
            u["waiting"] = "photo"
            return send_message(cid, "📸 عکس را بفرست:", chat_keyboard())

        if text == "📤 ارسال فیلم":
            u["waiting"] = "video"
            return send_message(cid, "🎥 فیلم را بفرست:", chat_keyboard())

        if text == "📤 ارسال موزیک":
            u["waiting"] = "audio"
            return send_message(cid, "🎵 MP3 یا فایل صوتی را بفرست:", chat_keyboard())

        if f:
            target = PARTNER_CHAT_ID if cid == OWNER_CHAT_ID else OWNER_CHAT_ID
            wanted = u.get("waiting")

            if wanted in (None, "photo", "video", "audio"):
                allowed = {
                    None: {"photo", "video", "audio", "voice", "document"},
                    "photo": {"photo"},
                    "video": {"video"},
                    "audio": {"audio"}
                }
                if f["type"] not in allowed[wanted]:
                    return send_message(cid, "❌ نوع فایل با گزینه انتخاب‌شده یکی نیست.", chat_keyboard())

                ok = send_file_to_chat(
                    target,
                    f,
                    cid == OWNER_CHAT_ID and target == PARTNER_CHAT_ID
                )
                u["waiting"] = None
                return send_message(
                    cid,
                    "✅ ارسال شد ❤️" if ok else "❌ ارسال ناموفق بود.",
                    chat_keyboard()
                )
        ...
        if u.get("waiting") == "chat_text" and text:
            ...
            ok = send_message(
                target,
                f"💬 پیام از {'عشقت' if cid == OWNER_CHAT_ID else 'پارتنرت'}:\n\n{text}"
            )
            u["waiting"] = None
            return send_message(
                cid,
                "✅ پیام ارسال شد ❤️" if ok else "❌ ارسال ناموفق بود.",
                chat_keyboard()
            )
    ...
    if text == "📸 عکس‌ها":
        if not u["photos"]:
            u["waiting"] = "password"
            return send_message(cid, "🔐 رمز را وارد کن.")
        return send_message(cid, "📸 عکس را انتخاب کن:", photo_keyboard())

    if u.get("waiting") == "password":
        if text == PASSWORD:
            u.update(photos=True, waiting=None)
            return send_message(cid, "🔓 گالری باز شد ❤️", photo_keyboard())
    ...
    if text in PHOTOS:
        if not u["photos"]:
            ...
            return send_message(cid, "🔐 رمز را وارد کن.")
        p = PHOTOS[text]
        return send_photo(cid, p["path"], p["caption"])

    if text == "📅 روز آشنایی":
        return send_message(cid, meeting_text())

    if text == "⏳ ساعت تا تولدت":
        n = now_iran()
        b = datetime.datetime(n.year, BIRTH_MONTH, BIRTH_DAY, tzinfo=IRAN_TZ)
        if n >= b:
            b = b.replace(year=n.year + 1)
        hours = int((b - n).total_seconds() // 3600)
        return send_message(cid, f"🎂 تا تولدت {hours:,} ساعت مانده ❤️")

    if text == "📊 وضعیت پارتنر" and cid == OWNER_CHAT_ID:
        d = PARTNER_ACTIVITY.get(PARTNER_CHAT_ID)
        if not d:
            return send_message(cid, "❌ هنوز تعاملی ثبت نشده.")
        return send_message(
            cid,
            f"📊 وضعیت پارتنر\n"
            f"🆔 {PARTNER_CHAT_ID}\n"
...
def handle_channel(cid, text, f, u):
    if text == "✍️ افزودن متن کانال":
        u["waiting"] = "channel_text"
        return send_message(cid, "✍️ متن را بفرست:", channel_keyboard())

    if text == "🎵 افزودن آهنگ کانال":
        u["waiting"] = "channel_audio"
        return send_message(cid, "🎵 آهنگ را به‌صورت Audio یا MP3 بفرست:", channel_keyboard())

    if text == "🎥 افزودن کلیپ کانال":
        u["waiting"] = "channel_video"
        return send_message(cid, "🎥 کلیپ را به‌صورت Video یا MP4 بفرست:", channel_keyboard())
    ...
    if text == "🗑 پاک کردن صف کانال":
        clear_kind()
        u["waiting"] = None
        return send_message(cid, "🗑 کل صف پاک شد.", channel_keyboard())

    if text == "⏰ تنظیم ساعت کانال":
        u["waiting"] = "channel_time"
        return send_message(cid, "⏰ ساعت را مثل 14:30 بفرست:", channel_keyboard())

    if text == "⚡ ارسال فوری کانال":
        u["waiting"] = "channel_now"
        return send_message(cid, "⚡ متن، آهنگ یا کلیپ را بفرست:", channel_keyboard())

    w = u.get("waiting")

    if w == "channel_time":
        try:
            h, m = map(int, text.replace("：", ":").split(":"))
            if not (0 <= h <= 23 and 0 <= m <= 59):
                raise ValueError
            set_setting("post_time", f"{h:02d}:{m:02d}")
            u["waiting"] = None
            return send_message(
                cid,
                f"✅ ساعت روی {h:02d}:{m:02d} تنظیم شد.",
                channel_keyboard()
            )
        except Exception:
            return send_message(cid, "❌ قالب نادرست است؛ نمونه: 14:30", channel_keyboard())

    if w == "channel_text":
        if not text:
            return send_message(cid, "❌ متن خالی است؛ متن را بفرست.", channel_keyboard())
        add_queue("text", text)
        u["waiting"] = None
        return send_message(cid, "✅ متن به صف اضافه شد.", channel_keyboard())

    if w in ("channel_audio", "channel_video", "channel_now"):
        if not f:
            return send_message(cid, "❌ فایل دریافت نشد؛ دوباره ارسال کن.", channel_keyboard())

        kind = f["type"]

        if w == "channel_audio":
            if kind != "audio":
                return send_message(cid, "❌ برای این بخش فقط آهنگ یا MP3 بفرست.", channel_keyboard())
            add_queue(
                "audio",
                f["file_id"],
                f.get("caption", ""),
            ...
            u["waiting"] = None
            return send_message(cid, "✅ آهنگ به صف اضافه شد.", channel_keyboard())

        if w == "channel_video":
            if kind != "video":
                return send_message(cid, "❌ برای این بخش فقط کلیپ یا ویدیو بفرست.", channel_keyboard())
            add_queue("video", f["file_id"], f.get("caption", ""))
            u["waiting"] = None
            return send_message(cid, "✅ کلیپ به صف اضافه شد.", channel_keyboard())
        ...
        if w == "channel_now":
            if kind == "audio":
                ...
                ok = send_video(CHANNEL_ID, f["file_id"], f.get("caption", ""))
            elif kind == "text":
                ok = send_message(CHANNEL_ID, f.get("text", ""))
            else:
                ok = False
            ...
            return send_message(
                cid,
                "✅ فوری در کانال منتشر شد." if ok else "❌ انتشار ناموفق بود.",
                channel_keyboard()
            )
...
def publish_daily():
    # هر روز به ترتیب: متن، آهنگ، کلیپ.
    # فقط بعد از موفقیت از صف حذف می‌شوند.
    for kind in ("text", "audio", "video"):
        ...
        if kind == "text":
            ok = send_message(CHANNEL_ID, item["content"])
        elif kind == "audio":
            ok = send_audio(
                CHANNEL_ID,
                item["content"],
        ...
        if ok:
            ...
            print("ارسال ناموفق، آیتم در صف باقی ماند:", item["id"], kind)
    ...
    while True:
        try:
            ...
            if n.hour == h and n.minute == m and last != today:
                publish_daily()
                set_setting("last_post_date", today)
                last = today
        except Exception as e:
            print("scheduler:", e)
        time.sleep(20)
...
def health():
    return jsonify(
        status="ok",
        channel=CHANNEL_ID,
        post_time=setting("post_time", "10:10"),
        queue=queue_counts()
    )
