...
# ============================================================
# 🎉 پیام روز دختر (1 سپتامبر) - با معذرت‌خواهی
# ============================================================

GIRLS_DAY_MESSAGE = """🎀 روز دختر مبارک...

میدونم بیست روزه که حرف نمیزنیم و سکوت بینمون سنگین شده.

امروز روز دختره... روزی که یادآوری میکنه تو چه دختر خاص و قشنگی هستی.

یه روزی یه دختر بچه بودی که دنیا براش تازگی داشت...
حالا شدی یه دختر که میتونه دنیای یه نفر رو تغییر بده.

خب... بیست روز سکوت واسه دوتایی که حرفای ناگفته زیاد دارن، زیاده.

نمیدونم این روزا برات چطور گذشته، ولی من هر روز یه کم بیشتر بهت فکر کردم.

🌹 روزت مبارک، نسا...

امیدوارم امروز لبخند بزنی، حتی اگه یادت نباشه که لبخندت چقدر قشنگه.
"""
...
def get_meeting_days():
    return get_meeting_seconds() // 86400
...
# ============================================================
# ⌨️ منوی اصلی - دکمه روز دختر فقط برای صاحب ربات
# ============================================================
...
def get_main_keyboard(chat_id=None):
    ...
    if str(chat_id) == YOUR_CHAT_ID:
        ...
        keyboard.append(["🎀 روز دختر"])  # فقط برای صاحب ربات
...
# ============================================================
# 🎀 منوی روز دختر (انتخاب مخاطب) - فقط برای صاحب ربات
# ============================================================
...
⚠️ توجه:
تلگرام به ربات Read Receipt واقعی برای عکس نمی‌دهد.
هر تعامل بعدی کاربر می‌تواند به عنوان نشانه تعامل ثبت شود."""
        )
...
def handle_message(
    ...
    # ============================================================
    # 🎀 ارسال به پارتنر (روز دختر)
    # ============================================================

    if text == "🎀 ارسال به پارتنر":

        if chat_id == YOUR_CHAT_ID:

            send_message(
                chat_id,
                "🎀 ارسال پیام روز دختر به پارتنر..."
            )
    ...
    if text == "🧪 ارسال به تست":
        ...
        if chat_id == YOUR_CHAT_ID:
            ...
            send_message(
                chat_id,
                "🧪 ارسال پیام روز دختر به اکانت تست..."
            )
    ...
    # --------------------------------------------------------
    # 👀 هر تعامل واقعی بعد از ارسال عکس
    # --------------------------------------------------------
...
⏰ {get_current_iran_time().strftime('%H:%M:%S')}"""
    ...
    if text == "🧪 ارسال به تست":
        ...
        if chat_id == YOUR_CHAT_ID:
            ...
            send_message(
                chat_id,
                "🧪 درخواست آشتی به اکانت تست ارسال شد!"
            )
...
if __name__ == "__main__":
    ...
    print(
        "💕 پیام بعد از ارسال عکس حذف شده است!"
    )

    print(
        "🎀 دکمه روز دختر فقط برای صاحب ربات نمایش داده میشود!"
    )
